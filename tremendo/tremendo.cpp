...
//		misc1<<delta_r*(i+1.)<<"   "<<real(funcion_regular[0].wf[i])<<"   "<<abs(potencial[i])<<"   "<<funcion_regular[0].energia<<endl;
	}
...
//		misc4<<funcion_regular[0].r[i]<<"   "<<real(funcion_regular[0].wf[i])<<"   "<<real(funcion_irregular[0].wf[i])<<endl;
//		misc2<<funcion_regular[0].r[i]<<"  "<<abs(funcion_regular[0].wf[i])<<"  "<<
//				abs(funcion_irregular[0].wf[i])<<endl;
	}
...
//		GeneraEstadosPI(&(parm->pot[indx_pot_a]),&(parm->st[indx_st]),parm->radio,parm->puntos,0.,parm,1,parm->m_b/(parm->m_b+1.));
		GeneraEstadosPI(&(parm->pot[indx_pot_a]),&(parm->st[indx_st]),parm->radio,parm->puntos,0.,parm,1,0.991935483870968,D0,rms);
...
//		GeneraEstadosPI(&(parm->pot[indx_pot_B]),&(parm->st[indx_st]),parm->radio,parm->puntos,0.,parm,1,parm->m_A/(parm->m_A+1.));
		GeneraEstadosPI(&(parm->pot[indx_pot_B]),&(parm->st[indx_st]),parm->radio,parm->puntos,0.,parm,1,2./3.,D0,rms);
							...
							for(P=abs((intS->final_st->l)-(ints->inicial_st->l));(P<=(intS->final_st->l)+(ints->inicial_st->l)) && (c2!=0.);P++)
							{
								...
								for(lc=abs(la-P);(lc<=la+P) && (c3!=0.) && (lc<parm->lmax);lc++)
								{
									...
									if(c4!=0.)
									{
										...
										misc4<<la<<"   "<<abs(*sgrande_mas)<<"   "<<abs(Clalb[la][lb][0])<<endl;
//										misc1<<la<<"   "<<abs(Clalb[la][lb][0])<<"  "<<
//												abs(*sgrande_mas)<<endl;
									}
								}
							}
						}
...
void Polarization(parametros* parm)
{
	...
	complejo* Inl=new complejo[num_rd];
...
{
	double rn,rnx,rnz,rp,rpx,rpz,rdp,rpn,sintheta,costheta,j0,VnB_int,theta,kd;
	int n1,n2,n3,n0;
	...
	for(n1=0;n1<dim_rd->num_puntos;n1++)
	{
		...
		for(n2=0;n2<dim_rpn->num_puntos;n2++)
		{
			...
			for(n3=0;n3<dim_theta->num_puntos;n3++)
			{
				...
				VnB_int=interpola_dbl(VnB->pot,VnB->r,rn,VnB->puntos);
				j0=gsl_sf_bessel_jl(0,k0*rp);
				for(n0=0;n0<num_rd;n0++)
				{
					if(rdp>=rd[n0]){
						f_rd=interpola_cmpx(f->wf,f->r,rd[n0],f->puntos);
						Inl[n0]+=f_rd*g_rdp*sintheta*stn_int*std_int*VnB_int*rdp*rpn*rpn*j0*dim_rd->pesos[n1]*
					...
					if(rdp<rd[n0]){
						...
						Inl[n0]+=g_rd*f_rdp*sintheta*stn_int*std_int*VnB_int*rdp*rpn*rpn*j0*dim_rd->pesos[n1]*
								dim_rpn->pesos[n2]*dim_theta->pesos[n3]*(dim_theta->b-dim_theta->a)
								*(dim_rpn->b-dim_rpn->a)*(dim_rd->b-dim_rd->a)/(kd*rd[n0]*64.*PI);
					}
				}
