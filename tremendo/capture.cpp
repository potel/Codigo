...
void Capture(struct parametros *parm)
{
	...
	for (n = 0; n < parm->num_cm; n++)
	{
		...
		if (parm->a_potcm == parm->pot[n].id)
			indx_pot_a = n;
		if (parm->B_potcm == parm->pot[n].id)
			indx_pot_B = n;
	}
	for (n = 0; n < parm->num_opt; n++)
	{
		if (parm->optico_ingreso == parm->pot_opt[n].id)
			indx_ingreso = n;
		if (parm->optico_salida == parm->pot_opt[n].id)
			indx_salida = n;
		if (parm->optico_intermedio == parm->pot_opt[n].id)
			indx_intermedio = n;
		if (parm->core_pot == parm->pot_opt[n].id)
			indx_core = n;
		if (parm->pot_transfer == parm->pot_opt[n].id)
			indx_transfer = n;
	}
	...
	for (n = 0; n < parm->a_numst; n++)
	{
		...
		{
			if (parm->a_estados[n] == parm->st[m].id)
				indx_st = m;
		}
		...
		{
			GeneraEstadosContinuo(&(parm->pot_opt[indx_scatt]), &(parm->st[indx_st]), parm->radio, parm->puntos, carga_trans * (carga_out), parm, parm->n1_masa * parm->m_b / (parm->n1_masa + parm->m_b));
		}
	...
	for (n = 0; n < parm->B_numst; n++)
	{
		for (m = 0; m < parm->num_st; m++)
		{
			if (parm->B_estados[n] == parm->st[m].id)
				indx_st = m;
		}
...
void AmplitudeCapture(struct parametros *parm)
{
	...
	fp12.open("dsdO.txt");
	ofstream fp13;
	fp13.open("dsdEdOdL.txt");
	ofstream fp14;
	fp14.open("T-coefficients.txt");
	ofstream fp15;
	fp15.open("yahfc-distribution.txt");
	ofstream fp16;
	fp16.open("T-coefficients_restricted_angle.txt");
	ofstream fp17;
	fp17.open("yahfc-distribution_restricted_angle.txt");
	...
	double *t_coeff_mas = new double[parm->lmax + 1];
	double *t_coeff_menos = new double[parm->lmax + 1];
	...
	int l, lp, ld, indx_salida, indx_ingreso, indx_core, indx_neutron_target, indx_st, n, 
	la, m, len, flag, n1, indx_transfer,count;
	...
	ifstream RIPL_fl;
	RIPL_fl.open("RIPL_potential-Madland.dat");
	...
	if (!strncmp(parm->unidades, "milib", len))
		flag = 1;
	if (!strncmp(parm->unidades, "fm2", len))
		flag = 2;
	if (!strncmp(parm->unidades, "b", len))
		flag = 3;
	if (!strncmp(parm->unidades, "microb", len))
		flag = 4;
	switch (flag)
	{
	case 1:
	...
	fp15<<ceil((parm->enerange_max-parm->enerange_min)/parm->enerange_step)+1<<endl;
	for (energia_out = parm->enerange_min; energia_out < parm->enerange_max; energia_out += parm->enerange_step)
	{
		...
		if (Ecm_old < 999.)
		{
			deltaE = abs(Ecm - Ecm_old);
			cout << "deltaE: " << deltaE << "    Ecm: " << Ecm << "     Ecm_old: " << Ecm_old << endl;
		}
		energia_trans = (parm->n1_masa + parm->T_masa) * Ecm / (parm->T_masa);
		cout << endl
			 << endl
			 << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
		cout << "Energy of detected cluster: " << energia_out << "  "
			 << "Energy of absorbed cluster: "
			 << energia_trans << endl;
		fp9 << energia_out << "  " << Ecm << "  ";
		...
		if (parm->koning_delaroche == 1)
		{
			for (n = 0; n < parm->puntos; n++)
			{
				...
				if (carga_trans > 0.1)
					parm->pot_opt[indx_neutron_target].pot[n] = pot_p;
				...
				if (carga_out < 0.1)
					parm->pot_opt[indx_salida].pot[n] = pot_n;
			}
		}
		if (parm->koning_delaroche == 10)
		{
		...
		for (l = parm->lmin; l < parm->ltransfer; l++)
		{
			...
			if (Ecm <= 0.)
				wronskiano_up = GeneraGreenFunctionLigada(&(funcion_regular_up[0]), &(funcion_irregular_up[0]),
			...
			if (l == 0)
			{
				funcion_irregular_down[0].j = parm->n_spin;
				funcion_regular_down[0].j = parm->n_spin;
				funcion_irregular_down[1].j = parm->n_spin;
				funcion_regular_down[1].j = parm->n_spin;
			}
			if (Ecm <= 0.)
				wronskiano_down = GeneraGreenFunctionLigada(&(funcion_regular_down[1]), &(funcion_irregular_down[1]),
															&(parm->pot_opt[indx_neutron_target]), parm->radio, parm->puntos, carga_trans * (parm->T_carga),
															parm->n1_masa * parm->T_masa / (parm->n1_masa + parm->T_masa), parm->n_spin);
			if (Ecm > 0.)
			{
			...
			for (lp = 0; lp < parm->lmax; lp++)
			{
				...
				for (ld = abs(l - lp); (ld <= l + lp) && (ld < parm->lmax); ld++)
				{
					...
					rhofac = (16. * pow(PI, 2.5) * pow(I, ld - lp) * pow(-1., l) *
							  exp_delta_coulomb_f[lp] * exp_delta_coulomb_i[ld] * sqrt(2. * ld + 1.)) /
							 (parm->k_Aa * k_p * sqrt(2. * l + 1.));
					...
					for (n = 0; n < dim1->num_puntos; n++)
					{
						...
						for (m = 0; m <= lp; m++)
						{
							...
							if (parm->prior == 1)
								non[n][l][m][lp] += (rhofac * ClebsGordan(lp, -m, ld, 0, l, -m) * nonm[0] * rn);
						}
					}
				}
				dim1->a = parm->r_Ccmin;
				dim1->b = parm->r_Ccmax;
				if (energia_trans > 0.)
					ElasticBreakup(Teb, rho, Ecm, &(parm->pot_opt[indx_neutron_target]),
			...
			// cout<<"abs: "<<rhoE*escala*sigma_const<<"  "<<
			// 					AbsorcionPrior(direct, non_orth, cross_term, &(parm->pot_opt[indx_neutron_target]),
			// 								   phi_up, non, dim1, l, parm->lmax, r_F);
			t_coeff_mas[l]=inc_break_lmas[l]*parm->k_Aa*parm->k_Aa/PI;
			inc_break[l] = inc_break_lmas[l];
			inc_break_lmenos[l] = rhoE * escala * sigma_const * AbsorcionPrior(direct, non_orth, cross_term, &(parm->pot_opt[indx_neutron_target]), phi_down, non, dim1, l, parm->lmax, r_F);
			t_coeff_menos[l]=inc_break_lmenos[l]*parm->k_Aa*parm->k_Aa/PI;
			inc_break[l] += inc_break_lmenos[l];
			if (energia_trans > 0.)
				elastic_break[l] = rhoE * rhoE_n * escala * sigma_const * PI * ElasticBreakupCross(Teb, l, parm->lmax);
			cross_total += inc_break[l];
			cross_total_elasticb += elastic_break[l];
			cross_total_breakup += total_break[l];
			cout << " NEB cross section: " << inc_break[l] << endl
				 << endl;
			cout << " EB cross section: " << elastic_break[l] << endl
				 << endl;
			fp9 << "  " << inc_break[l] << "  " << elastic_break[l] << "  ";
		}
		//cout<<t_coeff_mas[1]<<endl;
		//exit(0);
		T_coefficients(t_coeff_menos,t_coeff_mas,energia_trans,parm,&fp14,&fp15,parm->J_A,parm->parity_A);
		...
		fp9 << cross_total << "  " << cross_total_elasticb << "  " << cross_total + cross_total_elasticb << endl;
		cross_total = 0.;
		cross_total_elasticb = 0.;
		...
		if (parm->capture_angular == 1)
		{
			...
			for (n = 0; n < parm->cross_puntos; n++)
			{
				...
				if ((theta >= PI * parm->angle0 / 180.) && (theta <= PI * parm->angle1 / 180.))
				{
					...
					fp10 << theta * 180. / PI << "  " << sigma_const * escala * rhoE * cross << "  " << rhoE * rhoE_n * escala * sigma_const * PI * elastic_cross << "  " << sigma_const * escala * rhoE * (cross) + (rhoE * rhoE_n * escala * sigma_const * PI * elastic_cross) << endl;
					...
					fp13 << energia_out << "  " << Ecm<< "  " << theta * 180. / PI;
					for (l = 0; l < parm->ltransfer; l++)
					{
						...
						t_coeff_mas[l]=inc_break_lmas[l]*parm->k_Aa*parm->k_Aa/PI;
						t_coeff_menos[l]=inc_break_lmenos[l]*parm->k_Aa*parm->k_Aa/PI;
						fp13<<"  "<<sigma_const*escala*rhoE*(cross_up[l]+cross_down[l]);
					}
					fp13<<endl;
				}
			}
			...
			T_coefficients(t_coeff_menos,t_coeff_mas,energia_trans,parm,&fp16,&fp17,parm->J_A,parm->parity_A);
	...
	for (n1 = 0; n1 < dim1->num_puntos; n1++)
	{
		...
		for (n2 = 0; n2 < dim2->num_puntos; n2++)
		{
			...
			for (n3 = 0; n3 < dim2->num_puntos; n3++)
			{
				...
				{
					if (m <= lp)
						suma[m] += rAp * seno * seno_Bn * fl * gl * ud * vpn * FuncionAngular(lp, ld, l, m, l, m, coseno, coseno_d, coseno_Bn) *
				  ...
				  potencial_optico *core, int l, double rBn, parametros *parm, parametros_integral *dim1, parametros_integral *dim2)
{
	...
	rAn = km * rBn;
	N0 = 0.;
	// cout<<v->pot[0]<<"  "<<v->pot[1]<<"  "<<v->pot[2]<<endl;
	...
	for (n1 = 0; n1 < dim1->num_puntos; n1++)
	{
		...
		if (parm->remnant == 1)
			corepot = interpola_cmpx(core->pot, core->r, rAp, core->puntos);
		for (n2 = 0; n2 < dim2->num_puntos; n2++)
		{
			theta = (dim2->a) + ((dim2->b) - (dim2->a)) * ((dim2->puntos[n2]) + 1.) / 2.;
			//			theta=PI/2.;
			...
			rd = sqrt(rdx * rdx + rdz * rdz);
			if (parm->remnant == 1)
				inpot = interpola_cmpx(optico->pot, optico->r, rd, optico->puntos);
			rpnx = -rAp * seno;
			rpnz = rAn - rAp * coseno;
			rpn = sqrt(rpnx * rpnx + rpnz * rpnz);
			coseno_d = rdz / rd;
...
{
	if (l > lp + ld)
		Error("l>lp+ld en FuncionAngular");
	if (l < abs(lp - ld))
		Error("l<lp-ld en FuncionAngular");
	if (m > l)
		Error("m>l en FuncionAngular");
	if (m > lp)
		Error("m>lp en FuncionAngular");
	int mlp;
	complejo suma = 0.;
	if ((M >= 0) && (abs(M) <= ld))
		suma += ClebsGordan(lp, 0, ld, M, K, M) * gsl_sf_legendre_sphPlm(lp, 0, costheta) *
				gsl_sf_legendre_sphPlm(ld, M, costheta_d);
	if ((M < 0) && (abs(M) <= ld))
		suma += pow(-1., M) * ClebsGordan(lp, 0, ld, M, K, M) * gsl_sf_legendre_sphPlm(lp, 0, costheta) *
				gsl_sf_legendre_sphPlm(ld, M, costheta_d);
	for (mlp = 1; mlp <= lp; mlp++)
	{
		if ((M - mlp >= 0) && (abs(M - mlp) <= ld))
			suma += ClebsGordan(lp, mlp, ld, M - mlp, K, M) * gsl_sf_legendre_sphPlm(lp, mlp, costheta) *
					gsl_sf_legendre_sphPlm(ld, M - mlp, costheta_d);
		if ((M - mlp < 0) && (abs(M - mlp) <= ld))
			suma += pow(-1., M - mlp) * ClebsGordan(lp, mlp, ld, M - mlp, K, m) * gsl_sf_legendre_sphPlm(lp, mlp, costheta) *
					gsl_sf_legendre_sphPlm(ld, mlp - M, costheta_d);
		if ((M + mlp) >= 0 && (abs(M + mlp) <= ld))
			suma += pow(-1., mlp) * ClebsGordan(lp, -mlp, ld, m + mlp, K, M) * gsl_sf_legendre_sphPlm(lp, mlp, costheta) *
					gsl_sf_legendre_sphPlm(ld, M + mlp, costheta_d);
		if ((M + mlp < 0) && (abs(M + mlp) <= ld))
			suma += pow(-1., M) * ClebsGordan(lp, -mlp, ld, m + mlp, K, M) * gsl_sf_legendre_sphPlm(lp, mlp, costheta) *
					gsl_sf_legendre_sphPlm(ld, -mlp - M, costheta_d);
	}
	// suma=1.;
	if (-m >= 0)
		suma *= gsl_sf_legendre_sphPlm(l, -m, costheta_Bn);
	if (-m < 0)
		suma *= pow(-1., m) * gsl_sf_legendre_sphPlm(l, m, costheta_Bn);
...
{
	if (l > lp + ld)
		Error("l>lp+ld en FuncionAngular2");
	if (l < abs(lp - ld))
		Error("l<lp-ld en FuncionAngular2");
	int mlp;
	complejo suma = 0.;
	...
	{
		if (mlp <= ld)
			suma += pow(-1., mlp) * gsl_sf_legendre_sphPlm(lp, mlp, costheta) *
...
{
	if (l > lp + ld)
		Error("l>lp+ld en FuncionAngular3");
	if (l < abs(lp - ld))
		Error("l<lp-ld en FuncionAngular3");
	int mlp;
	complejo suma = 0.;
	if (abs(m) <= ld)
		suma += ClebsGordan(lp, 0, ld, m, l, m) * SphericalHarmonic(lp, 0, costheta, phi) * SphericalHarmonic(ld, m, costheta_d, phi_d);
	for (mlp = 1; mlp <= lp; mlp++)
	{
		if (abs(m - mlp) <= ld)
			suma += ClebsGordan(lp, mlp, ld, m - mlp, l, m) * SphericalHarmonic(lp, mlp, costheta, phi) *
...
complejo SphericalHarmonic(int l, int m, double costheta, double phi)
{
	...
	if (m > l)
	{
		cout << "Cuidado, m>l en SphericalHarmonic. Se devuelve 0." << endl;
		return 0.;
	}
	if (m >= 0)
		valor = gsl_sf_legendre_sphPlm(l, m, costheta) * exp(I * double(m) * phi);
	if (m < 0)
		valor = pow(-1., m) * gsl_sf_legendre_sphPlm(l, -m, costheta) * exp(I * double(m) * phi);
	return valor;
}
								   ...
								   double radio_max, int puntos, double q1q2, double masa, double spin)
{
	...
	for (i = 0; i < puntos; i++)
	{
		...
		if (regular->r[i] > potencial->radio_coul)
			v[i] = potencial->pot[i] + E_CUADRADO * q1q2 / regular->r[i];
		if (potencial->r[i] <= potencial->radio_coul)
			v[i] = potencial->pot[i] + E_CUADRADO * q1q2 * (3. - (regular->r[i] / potencial->radio_coul) * (regular->r[i] / potencial->radio_coul)) / (2. * potencial->radio_coul);
		vs[i] = -2. * (potencial->Vso) * exp((regular->r[i] - potencial->radioso) / potencial->aso) /
				(regular->r[i] * potencial->aso * (1. + exp((regular->r[i] - potencial->radioso) / potencial->aso)) * (1. + exp((regular->r[i] - potencial->radioso) / potencial->aso)));
	...
	{
		irregular->wf[i - 1] = (2. * (1. - 0.416666667 * dd * (-vv[i] + Etrial)) * irregular->wf[i] - (1. + 0.083333333 * dd * (-vv[i + 1] + Etrial)) * irregular->wf[i + 1]) / (1. + 0.083333333 * dd * (-vv[i - 1] + Etrial));
		if (real(irregular->wf[i + 1]) * real(irregular->wf[i]) < 0.)
			ND = ND + 1;
	}
	...
	for (i = 1; i < puntos - 1; i++)
	{
		...
		if (real(regular->wf[i + 1]) * real(regular->wf[i]) < 0.)
			ND = ND + 1;
	}
...
///////////////////////////////////////////////////////////////////
///    Transmission coefficients as a function of spin and energy,
///     to be used by YAHFC.
//////////////////////////////////////////////////////////////////
void T_coefficients(double *lmenos, double *lmas, double energia_trans, parametros *parm, ofstream *fp3, ofstream *fp4, double s, int parity)
{
	int l;
	double Ex, factor, J, Jp, j_menos, j_mas, norma,small;
	double *parity1 = new double[parm->ltransfer + int(ceil(s)) + 2];
	double *parity2 = new double[parm->ltransfer + int(ceil(s)) + 2];
	Ex = energia_trans - parm->B_Sn;
	*fp3 << "NEnergy= " << Ex << "   Entries " << parm->ltransfer + int(ceil(s)) + 2 << endl;
	...
	cout<<"spin :"<<s<<endl;
	*fp4<<Ex<<" "<<0.0<<"  "<<1<<endl;
	*fp4<<2.*(parm->ltransfer + int(ceil(s)) + 2)<<endl;
	//s=2;
	small=0.5;
	if (floor(s+0.5)==s+0.5) small=0.;
	//cout<<"s: "<<s+0.5<<"  floor: "<<floor(s+0.5)<<"  small: "<<small<<endl;
	//exit(0);
	for (l = 0; l < parm->ltransfer + int(ceil(s)) + 2; l++)
	{
		parity1[l] = 0.;
	...
	for (Jp = 0; Jp < parm->ltransfer + int(ceil(s)) + 2; Jp++)
	{
		if (parity > 0)
		{
			...
			*fp3 << std::fixed;
			fp3->precision(7);
			*fp3 << setw(12) << left << parity2[int(Jp)] / norma << left << setw(12) << parity1[int(Jp)] / norma << endl;
			*fp4<<Jp+small<<"  "<<1<<"  "<<parity2[int(Jp)]<<endl;
			*fp4<<Jp+small<<"  "<<-1<<"  "<<parity1[int(Jp)]<<endl;
		}
		if (parity < 0)
			*fp3 << left << setw(5) << Jp << setw(12) << left << parity1[int(Jp)] / norma << left << setw(12) << parity2[int(Jp)] / norma << endl;
	}
...
void AmplitudeClusterInelastic(struct parametros *parm)
{
	...
	for (n = 0; n < parm->num_st; n++)
	{
		...
		if (parm->a_estados[0] == parm->st[n].id)
			st1 = &(parm->st[n]);
		if (parm->a_estados[1] == parm->st[n].id)
			st2 = &(parm->st[n]);
	}
	len = strlen(parm->unidades);
	if (!strncmp(parm->unidades, "milib", len))
	...
	// exit(0);
	misc2 << "& Multipolar potential" << endl
		  << endl;
	// for(n=0;n<dim1->num_puntos;n++)
	//   {
	//     misc2<<R[n];
	...
	//   }
	// exit(0);
	misc3 << "& Multipolar transition density" << endl
		  << endl;
	for (n = 0; n < dim1->num_puntos; n++)
	{
		misc3 << R[n];
			...
			for (lb = 0; lb < parm->lmax; lb++)
			{
				for (mf = -dmf; mf <= dmf; mf++)
				{
					...
					for (mi = -dmi; mi <= dmi; mi++)
					{
						...
						{
							if (M >= 0)
								Ttheta[mm][MM][n] += Slmm[lb][mm][MM] *
													 gsl_sf_legendre_sphPlm(lb, abs(M), costheta);
							if (M < 0)
								Ttheta[mm][MM][n] += pow(-1., M) * Slmm[lb][mm][MM] *
													 gsl_sf_legendre_sphPlm(lb, abs(M), costheta);
						}
					}
				}
	...
	for (n = 0; n < puntos; n++)
	{
		for (mf = -dmf; mf <= dmf; mf++)
		{
			...
			for (mi = -dmi; mi <= dmi; mi++)
			{
				...
				if (n < 1)
					alphasum[mmf][mmi] += abs(alpha[mmf][mmi][n]);
			}
		}
		for (m1 = -dm; m1 <= dm; m1++)
		{
...
void SimpleRho(double li, double lf, double K, int lg, complejo ***rhomm, int puntos, double theta1, double theta2)
{
	...
	for (n = 0; n < puntos; n++)
	{
		...
		for (mi = 0; mi <= li; mi++)
		{
			...
			{
				if (mi - mf >= 0)
					alpha[mf][mi][n] = ClebsGordan(li, mi, lf, -mf, K, mi - mf) *
									   gsl_sf_legendre_sphPlm(lg, mi - mf, cos(PI * theta / 180.));
				if (mi - mf < 0)
					alpha[mf][mi][n] = pow(-1., mi - mf) * ClebsGordan(li, mi, lf, -mf, K, mi - mf) *
...
void RIPL_potential::Get(double energy)
{
	...
	misc2 << energy << "  " << V1 << "  " << W1 << "  " << VD1 << "  " << WD1 << "  " << VSO1 << "  " << RV1 << "  " << RW1 << "  " << RVD1 << "  " << AV1
		  << "  " << AVD1 << endl;
	fhf = 0.;
	fv = 0.;
	gs = 0.;
	...
	for (n = 0; n < pot->puntos; n++)
	{
		...
		rsw = RVD1 * pow(A, 0.33333333333333333);
		fsw = 1. / (1. + exp((pot->r[n] - rsw) / AWD1));
		if (AWD1 != 0.)
			gsw = 4. * fsw * fsw * exp((pot->r[n] - rsw) / AWD1);
		...
		// cout << n << "  " << fhf << "  " << fv << "  " << gs << endl;
		// exit(0);
		pot->pot[n] = -V1 * fhf - (DWV1 + I * W1) * fv - VD1 * gs - I * WD1 * gsw;
		pot->Vso = VSO1;
		pot->rso = RVSO1;
		pot->aso = AVSO1;
		if (abs(VSO1)<1.e-5)  AVSO1=0.65;
		// cout << pot->r[n] << "  " << pot->pot[n] << endl;
	}
}
