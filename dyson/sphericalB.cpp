...
//**********************************************
  /** calculates an array (II) of the modified spherical bessels of first kind;
   \param lmax maximum l for which the value is calculated
   \param x independent variable
  */

void sphericalB::I(int lmax, double x)
{
  if (x == 0.)
    {
      II[0] = 1.;
      for (int l=1;l<=lmax;l++) II[l] = 1.;
      return;
    }
  II[0] = sinh(x)/x;
  II[1] = (x*cosh(x) - sinh(x))/pow(x,2);
  if (lmax >= 150) 
    {
      cout << "II array not big enough in sphericalB" << endl;
      cout << "full " << lmax << " " << x << endl; 
      abort();
    }
  for (int l=2;l<= lmax;l++)
    {
      II[l] = II[l-2] - (double)(2*l-1)/x*II[l-1];
    }

}
//********************************************************
  /** calculates an array (II) of the modified spherical bessels of first kind
   * in the asymptotic limit x approaches zero. The actual result is
   * the value in the array times exp(x)/2./x
   \param lmax maximum l for which the value is calculated
   \param x independent variable
  */
void sphericalB::asymptoticI(int lmax, double x)
{
