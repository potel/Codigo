 ...
 * step function
 ...
 \param Ep0 gap width, \f$W=0\f$ for \f$|E-E_{fermi}| < E_{p} \f$
 \param Efermi0 is the Fermi Energy in MeV
 \param m0 is the power, either 2 or 4 are allowed
...
double imaginaryForm::ImaginaryPotential(double E)
{
   ...
   //if(E < -200 && asymmetric) cout<<"A = "<<A<<", sym = "<<pot<<endl;
   if (asymmetric){
      //if(E<-200) cout<<"A = "<<A<<" asym = "<<AsymmetricImaginaryPotential(E)<<endl;
      pot += AsymmetricImaginaryPotential(E);
   }
   //if(E < -200 && asymmetric) cout<<"A = "<<A<<", pot = "<<pot<<endl;
...
double imaginaryForm::DerDeltaRealPotential(double E)
{
   double out=0.;
   if (C == 0.)
   {
      if (m == 2) out =  DerDeltaZero2Hole( E ) + DerDeltaZero2Particle(E);
      else if (m == 4) out = DerDeltaZero4Hole( E ) + DerDeltaZero4Particle(E);
   }
   else
   {
      if (m==2) out = DerDelta2Hole( E ) + DerDelta2Particle(E);
      else if (m==4) out = DerDelta4Hole( E ) + Delta4Particle(E);
   }
   if (asymmetric) {

      out += DerDeltaAsymmetricHole( E ) + DerDeltaAsymmetricParticle(E);
   }
